...
Many mirror sysadmins running [[http:	//httpd.apache.org/][Apache HTTP Server]] find the [[http://bwmod.sourceforge.net/][Bandwidth Mod]] useful for this purpose. Here is an example config that will do the following:
...
If run [[http://www.lighttpd.net/][lighttpd]] you can use the following:
<pre>$HTTP["url"] =~ "\.cvd$" {
...
if you run [[http://nginx.net/][Nginx]] , you can use the following (without simultaneous connections limit):
...
The Source is available at <a href="http://cband.linux.pl/download/" title="http://cband.linux.pl/download/">http://cband.linux.pl/download/</a> or <a href="http://sourceforge.net/projects/cband/" title="http://sourceforge.net/projects/cband/">http://sourceforge.net/projects/cband/</a>
...
Add the module to your apache-config by manual adding it to /etc/apache2/httpd.conf
...
This limits the downloadspeed to 20kb/s, allows 100 requests/second and a max of 300 connections.
...
With mod_cband you can also limit the downloadspeed based on monthly traffic or the source-ip. For more information see <a href="http://codee.pl/cband_documentation.html" title="http://codee.pl/cband_documentation.html">http://codee.pl/cband_documentation.html</a>
...
An alternative script is available here:
   * [[%ATTACHURL%/clam-getbl][clam-getbl]]: clam-getbl - a script by Imre Gergely
When run from crontab, instead of replacing the existing lighttpd configuration, it will just send an email asking you to do that.
...
Requirements:
   ...
   * [[http://www.balabit.com/][syslog-ng]]
   * [[http://netfilter.org/projects/iptables/index.html][iptables]]
...
Shell:<br /> mknod /var/log/apache/access.log p

Apache HTTP Server (httpd.conf or vhost.conf):<br />The logformat and the accesslog must be defined like
...
If you replace the &ldquo;&ndash;rcheck&rdquo; here with an &ldquo;&ndash;update&rdquo; statement, the block will last even longer. The &ldquo;&ndash;rcheck&rdquo; option means: we will block you the next hour. While &ldquo;&ndash;update&rdquo; means: we don&rsquo;t want to see you for an hour, but if we see you again during this time, we&rsquo;ll block you again. It means that you actually need to be quiet for 60 minutes to be able to log in again.
...
Use<br /> chmod 600 /sys/module/xt_recent/parameters/ip_list_tot<br /> echo 10000 &gt; /sys/module/xt_recent/parameters/ip_list_tot<br /> chmod 400 /sys/module/xt_recent/parameters/ip_list_tot<br />to change ip_list_tot "on-the-fly"
